...
public abstract class AbstractColumnSorter extends ViewerComparator {
	...
	public void activate() {
		...
		if (getViewer().getComparator() != null
				&& getViewer().getComparator() == AbstractColumnSorter.this) {
	...
	protected void setSorter(int direction) {
		if (direction == NONE) {
			...
			this.updater.setSortDirection(getViewer(),
					direction == ASC ? SWT.UP : SWT.DOWN);
	...
	public void dispose() {
		if (this.column != null && this.updater != null) {
			this.updater.removeListener(this.column, SWT.Selection,
					this.listener);

...
public class ColorUtil {
    ...
    public static void adapt(CTabFolder tabFolder) {
        ...
        if (!JFaceResources.getColorRegistry().hasValueFor(ACTIVE_TAB_START)) {
            JFaceResources.getColorRegistry().put(ACTIVE_TAB_START, getActiveTabStartColor());
            JFaceResources.getColorRegistry().put(ACTIVE_TAB_END, getActiveTabEndColor());
            JFaceResources.getColorRegistry().put(ACTIVE_TAB_TEXT, getActiveTabTextColor());
        }

        tabFolder.setSelectionBackground(new Color[] {
                JFaceResources.getColorRegistry().get(ACTIVE_TAB_START),
                JFaceResources.getColorRegistry().get(ACTIVE_TAB_END) }, new int[] { 100 }, true);
        tabFolder.setSelectionForeground(JFaceResources.getColorRegistry().get(ACTIVE_TAB_TEXT));
    ...
    private static RGB getActiveTabEndColor() {
        return Display.getCurrent().getSystemColor(SWT.COLOR_LIST_SELECTION).getRGB();
    }

    private static RGB getActiveTabStartColor() {
        if (Display.getCurrent().getDepth() < 15) return getActiveTabEndColor();
        RGB white = new RGB(255, 255, 255);
        Color c1 = Display.getCurrent().getSystemColor(SWT.COLOR_LIST_SELECTION);
        return blend(white, c1.getRGB(), 75);
    }

    private static RGB getActiveTabTextColor() {
        return Display.getCurrent().getSystemColor(SWT.COLOR_LIST_SELECTION_TEXT).getRGB();
    }
}

...
public class ColumnSupport {
    ...
    private static class BooleanCellEditor extends CheckboxCellEditor {
        ...
        public void activate() {
            this.value.setValue(this.value.getValue() == null || Boolean.TRUE.equals(this.value.getValue()) ? Boolean.FALSE : Boolean.TRUE);
            setValueValid(true);
     ...
     */
    public static ColumnSupport create(TableViewer viewer, int style, String columnLabel, IObservableSet knownElements, IValueProperty property) {
        return new ColumnSupport(new TableViewerColumn(viewer, style), new TableViewerUpdater(), columnLabel, knownElements, property);
    }
     ...
     * @return a new instance of {@link ColumnSupport} for chaining.
     */
    public static ColumnSupport create(TableViewer viewer, String columnLabel, IObservableSet knownElements, IValueProperty property) {
     ...
     * @return a new instance of {@link ColumnSupport} for chaining.
     */
    public static ColumnSupport create(TreeViewer viewer, int style, String columnLabel, IObservableSet knownElements, IValueProperty property) {
        return new ColumnSupport(new TreeViewerColumn(viewer, style), new TreeViewerUpdater(), columnLabel, knownElements, property);
    }
     ...
     * @return a new instance of {@link ColumnSupport} for chaining.
     */
    public static ColumnSupport create(TreeViewer viewer, String columnLabel, IObservableSet knownElements, IValueProperty property) {
        return create(viewer, SWT.NONE, columnLabel, knownElements, property);
    }
     ...
     */
    private ColumnSupport(ViewerColumn column, ViewerColumnUpdater updater, String columnLabel, IObservableSet knownElements, IValueProperty property) {
     ...
     * @return same object for chaining
     */
    public ColumnSupport addCheckboxEditingSupport(final DataBindingContext dbc, final IValueProperty property) {
     ...
     * @return same object for chaining
     */
    public ColumnSupport addPropertySorting(IValueProperty property, Comparator comparator) {
        if (this.sorter != null) {
            this.sorter.dispose();
        }
        final IValueProperty finalProperty = property != null ? property : this.property;
        final Comparator finalComparator = comparator != null ? comparator : naturalComparator();
        this.sorter = new AbstractColumnSorter(this.column, this.updater) {
            @Override
            protected int doCompare(Viewer viewer, Object e1, Object e2) {
                return finalComparator.compare(finalProperty.getValue(e1), finalProperty.getValue(e2));
            }
        };
        return this;
    }
     ...
     * @return same object for chaining
     */
    public ColumnSupport addTextEditingSupport(DataBindingContext dbc, IValueProperty property) {
    ...
    public ColumnSupport addTextEditingSupport(
            ...
            final UpdateValueStrategy targetToModel,
            final UpdateValueStrategy modelToTarget) {
        return addTextEditingSupport(dbc, SWT.NONE, property, targetToModel, modelToTarget);
    }
    ...
    public ColumnSupport addViewerEditingSupport(
            ...
            final UpdateValueStrategy targetToModel,
            final UpdateValueStrategy modelToTarget) {
        return addViewerEditingSupport(dbc, property, null, cellEditor, targetToModel, modelToTarget);
    }
    ...
    public ColumnSupport addViewerEditingSupport(
            final DataBindingContext dbc,
            final IValueProperty property,
            final IValueProperty canEditProperty,
            final CellEditor cellEditor,
            final UpdateValueStrategy targetToModel,
            final UpdateValueStrategy modelToTarget) {
        ...
        if (cellEditorViewer == null) {
            throw new IllegalArgumentException("Can't determine cell editor's viewer."); //$NON-NLS-1$
        }
        ...
        this.column.setEditingSupport(new ObservableValueEditingSupport(this.column.getViewer(), dbc) {
            ...
            @Override
            protected IObservableValue doCreateCellEditorObservable(CellEditor cellEditor) {
                return ViewerProperties.singleSelection().observe(cellEditorViewer);
            }
     ...
     *             when the number of children is different than 1
     */
    public ColumnSupport setLayoutData(ColumnLayoutData data) {
        Composite comp = getComposite();
        Widget column = this.updater.getColumn(this.column);
        Layout layout = comp.getParent().getLayout();
        if (layout != null && layout instanceof AbstractColumnLayout && comp.getParent().getChildren().length == 1) {
            ((AbstractColumnLayout) layout).setColumnData(column, data);
            return this;
        }
        if (comp.getParent().getChildren().length == 1) {
            comp.getParent().setLayout(layout = this.updater.createColumnLayout());
            if (layout != null) {
                ((AbstractColumnLayout) layout).setColumnData(column, data);
            }
            return this;
    ...
    /**
     * Used in cases where the width of the column is set by pixels
     * 
     * @param width
     *            the width of the column in pixels
     * @return
     */
    public ColumnSupport setPixelLayoutData(int width) {
        return setLayoutData(new ColumnPixelData(width, true, true));
    }
    ...
    public ColumnSupport setVisible(boolean visible) {
        ...
        // Set visibility
        if (visible) {
            width = getData(DEFAULT_COLUMN_WIDTH) != null ? ((Integer) getData(DEFAULT_COLUMN_WIDTH)).intValue() : 70;
            setWidth(width);
            setResizable(true);
        } else {
            setWidth(0);
    ...
    /**
     * Used in cases where the width of the column is set by weight
     * 
     * @param weight
     ...
     * @return
     */
    public ColumnSupport setWeightLayoutData(int weight, int min) {
        return setLayoutData(new ColumnWeightData(weight, min));
    }

...
public class SashFactory {
	...
	public static void createRightPane(final Composite parent,
		...
		Control main = parent.getChildren()[0];
		main.setLayoutData(new GridData(GridData.FILL_BOTH));

		// Get reference to the pane.
		Control trayControl = parent.getChildren()[1];
			...
			public void handleEvent(Event event) {
				if (event.detail != SWT.DRAG) {
					...
					int newWidth = clientArea.width
							- event.x
							- (sash.getSize().x + (rightSeparator != null ? rightSeparator
									.getSize().x : 0));
